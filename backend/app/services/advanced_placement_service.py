...
class AdvancedPlacementService:
    ...
    def place_items(self, items: List[ItemBase], containers: List[ContainerBase]) -> Tuple[List[PlacementItem], List[RearrangementStep]]:
        ...
        """
        logger.warning(f"ADVANCED PLACEMENT SERVICE ACTIVE - Placing {len(items)} items in {len(containers)} containers")
        ...
        # Ensure all placements are saved to the database
        if placements:
    ...
    def _find_optimal_position(self, item: Dict, container: Dict) -> Optional[Tuple[float, float, float, float, float, float]]:
        ...
        """
        logger.info(f"Finding optimal position for item {item.get('itemId', 'unknown')} in container {container.get('containerId', 'unknown')}")
        ...
        container_depth = int(container["depth"])
        container_height = int(container["height"])
        
        # Get existing item positions to avoid clustering
        existing_positions = []
        for item_data in container["items"]:
            if "position" in item_data:
                x, y, z = item_data["position"][0], item_data["position"][1], item_data["position"][2]
                existing_positions.append((int(x), int(y), int(z)))
        
        # Force horizontal distribution by defining fixed starting positions
        # based on how many items are already in the container
        num_existing_items = len(existing_positions)
        
        # Define candidate starting positions based on the container dimensions
        # These are strategic points distributed across the container
        candidate_positions = [
            (0, 0),                                                  # Front left
            (container_width - 1, 0),                               # Front right
            (0, container_depth - 1),                               # Back left
            (container_width - 1, container_depth - 1),             # Back right
            (container_width // 2, 0),                              # Front center
            (0, container_depth // 2),                              # Middle left
            (container_width - 1, container_depth // 2),            # Middle right
            (container_width // 2, container_depth - 1),            # Back center
            (container_width // 2, container_depth // 2),           # Center
            (container_width // 4, container_depth // 4),           # Front left quadrant
            (3 * container_width // 4, container_depth // 4),       # Front right quadrant
            (container_width // 4, 3 * container_depth // 4),       # Back left quadrant
            (3 * container_width // 4, 3 * container_depth // 4),   # Back right quadrant
        ]
        
        # Filter out positions that are too close to existing items
        min_distance = 5  # Minimum distance between items
        available_positions = []
        
        for pos_x, pos_y in candidate_positions:
            is_too_close = False
            for ex_x, ex_y, _ in existing_positions:
        ...
        # If all positions are too close, relax the distance constraint
        if not available_positions and candidate_positions:
            available_positions = candidate_positions
        
        # Prioritize positions based on item index or properties
        # For now, just use a simple index-based selection to ensure items spread out
        preferred_index = num_existing_items % max(1, len(available_positions))
        if available_positions:
            preferred_position = available_positions[preferred_index]
            logger.info(f"Preferred start position for item {item['itemId']}: {preferred_position}")
        else:
            # Fallback to a position derived from the item ID if no positions available
            item_id_hash = hash(str(item.get('itemId', 'default'))) % 1000
            preferred_position = (
                (item_id_hash % container_width) % max(1, container_width - int(item["width"])),
                ((item_id_hash // container_width) % container_depth) % max(1, container_depth - int(item["depth"]))
            )
            logger.info(f"Using fallback position for item {item['itemId']}: {preferred_position}")
        
        # For each orientation, try to place the item at the preferred position first
        for w, d, h in orientations:
            # Check if item fits in container with this orientation
            if w > container_width or d > container_depth or h > container_height:
                continue
            
            # Try the preferred position first
            i, j = preferred_position
            
            # Make sure the item doesn't extend beyond container bounds
            i = min(i, container_width - w)
            j = min(j, container_depth - d)
            i = max(0, i)
            j = max(0, j)
            
            # Find the best height for this position
            k_values = self._find_valid_heights(container, i, j, w, d, h)
            
            if k_values:
                # We found a valid position at the preferred location!
                k = k_values[0]  # Start with the lowest valid height
                best_position = (float(i), float(j), float(k), w, d, h)
                logger.info(f"Found valid position at preferred location: ({i},{j},{k})")
                break
            
            # If the preferred position didn't work, try a systematic search pattern
            logger.info(f"Preferred position not valid, trying systematic search")
            
            # Try all positions in the container
            for i in range(container_width - int(w) + 1):
                for j in range(container_depth - int(d) + 1):
                    # Find heights where the item can be placed
                    k_values = self._find_valid_heights(container, i, j, w, d, h)
                    ...
                    for k in k_values:
                        ...
                        # Add a bonus for positions far from existing items
                        if existing_positions:
                            min_distance_sq = min((i-x)**2 + (j-y)**2 for x, y, _ in existing_positions)
                            spread_bonus = min(0.5, min_distance_sq / 100)  # Cap the bonus
                            score += spread_bonus
                        
                        # Add a small bonus for positions away from the origin (0,0)
                        origin_distance = (i + j) / (container_width + container_depth)
                        score += 0.1 * origin_distance
                        
                        if score > best_score:
                            best_score = score
                            best_position = (float(i), float(j), float(k), w, d, h)
                            logger.debug(f"New best position: ({i},{j},{k}) with score {best_score}")
        
        if best_position:
            logger.info(f"Optimal position found at ({best_position[0]},{best_position[1]},{best_position[2]})")
        else:
            logger.warning(f"No valid position found for item {item.get('itemId', 'unknown')}")
        ...
        try:
            for placement in placements:
                ...
                if db_item:
                    ...
                    logger.debug(f"Saving placement of item {placement.itemId} to container {placement.containerId} at position ({placement.position.startCoordinates.width}, {placement.position.startCoordinates.depth}, {placement.position.startCoordinates.height})")
